...
// FIX: Corrected type imports to remove deprecated members and add necessary ones for streaming fix.
import { GoogleGenAI, Modality, Type, Part, Chat, GenerateContentResponse, Tool, Video, FunctionCall, GroundingChunk } from '@google/genai';
...
const voiceInputButton = document.getElementById('voice-input-button') as HTMLButtonElement;
const sendButton = form.querySelector('button[type="submit"]') as HTMLButtonElement;
const googleSearchToggle = document.getElementById('google-search-toggle') as HTMLInputElement;
const slashCommandHelper = document.getElementById('slash-command-helper') as HTMLDivElement;

// --- Attachment & File Elements ---
const attachFileButton = document.getElementById('attach-file-button') as HTMLButtonElement;
const attachmentMenu = document.getElementById('attachment-menu') as HTMLDivElement;
const attachImageButton = document.getElementById('attach-image-button') as HTMLButtonElement;
const attachVideoButton = document.getElementById('attach-video-button') as HTMLButtonElement;
...
// --- Stop Generation ---
const stopGenerationContainer = document.getElementById('stop-generation-container') as HTMLDivElement;
...
const chatTitleElement = document.getElementById('chat-title') as HTMLHeadingElement;
const menuToggle = document.getElementById('menu-toggle') as HTMLButtonElement;
const sidebar = document.getElementById('sidebar') as HTMLElement;
const headerTitleDiv = document.querySelector('.header-title') as HTMLDivElement;
const personaDisplay = document.getElementById('persona-display') as HTMLSpanElement;
...
const promptStartersContainer = welcomeScreen.querySelector('.prompt-starters') as HTMLDivElement;
...
const customInstructionContainer = document.getElementById('custom-instruction-container') as HTMLDivElement;
const systemInstructionInput = document.getElementById('system-instruction') as HTMLTextAreaElement;
const thinkingModeSelect = document.getElementById('thinking-mode') as HTMLSelectElement;
const responseLengthSelect = document.getElementById('response-length') as HTMLSelectElement;

// --- Creative Studio Elements ---
const openCreativeStudioButton = document.getElementById('open-creative-studio') as HTMLButtonElement;
const creativeStudioModal = document.getElementById('creative-studio-modal') as HTMLDivElement;
const closeCreativeStudioButton = document.getElementById('close-creative-studio') as HTMLButtonElement;
const studioInitialState = document.getElementById('studio-initial-state') as HTMLDivElement;
const studioLoadingState = document.getElementById('studio-loading-state') as HTMLDivElement;
const studioResultState = document.getElementById('studio-result-state') as HTMLDivElement;
const studioImageDropZone = document.getElementById('studio-image-drop-zone') as HTMLDivElement;
const studioImagePreview = document.getElementById('studio-image-preview') as HTMLImageElement;
const studioRemoveImageButton = document.getElementById('studio-remove-image') as HTMLButtonElement;
const studioPromptInput = document.getElementById('studio-prompt') as HTMLTextAreaElement;
const studioGenerateButton = document.getElementById('studio-generate-button') as HTMLButtonElement;
const studioImageInput = document.getElementById('studio-image-input') as HTMLInputElement;
const studioLoadingMessage = document.getElementById('studio-loading-message') as HTMLParagraphElement;
const studioResultVideo = document.getElementById('studio-result-video') as HTMLVideoElement;
const studioDownloadButton = document.getElementById('studio-download-button') as HTMLButtonElement;
const studioAddToChatButton = document.getElementById('studio-add-to-chat-button') as HTMLButtonElement;

// --- Image Editor Elements ---
const imageEditorModal = document.getElementById('image-editor-modal') as HTMLDivElement;
const closeImageEditorButton = document.getElementById('close-image-editor-button') as HTMLButtonElement;
const editorInitialState = document.getElementById('editor-initial-state') as HTMLDivElement;
const editorLoadingState = document.getElementById('editor-loading-state') as HTMLDivElement;
const editorResultState = document.getElementById('editor-result-state') as HTMLDivElement;
const editorOriginalImage = document.getElementById('editor-original-image') as HTMLImageElement;
const editorEditedImage = document.getElementById('editor-edited-image') as HTMLImageElement;
const editorPrompt = document.getElementById('editor-prompt') as HTMLTextAreaElement;
const editorGenerateButton = document.getElementById('editor-generate-button') as HTMLButtonElement;
const editorResultOriginalImage = document.getElementById('editor-result-original-image') as HTMLImageElement;
const editorResultEditedImage = document.getElementById('editor-result-edited-image') as HTMLImageElement;
const editorDownloadButton = document.getElementById('editor-download-button') as HTMLButtonElement;
const editorAddToChatButton = document.getElementById('editor-add-to-chat-button') as HTMLButtonElement;
...
// FIX: Corrected type from FunctionDeclarationTool[] to Tool[]
const TOOLS: Tool[] = [
  {
...
let lastUserPrompt: Part[] | null = null;
let editingMessage: EditingMessageState | null = null;

let attachedFiles: AttachedFile[] = [];
let isGenerating: boolean = false;
let isConversationModeActive: boolean = false;
let currentSpokenBubble: HTMLElement | null = null;

// --- Creative Studio State ---
let studioImageFile: File | null = null;
let studioGeneratedVideoBlob: Blob | null = null;

// --- Image Editor State ---
let editorSourceImage: { mimeType: string; data: string } | null = null;
let editorGeneratedImage: { mimeType: string; data: string } | null = null;
...
function extractSources(text: string): { cleanText: string; sources: {uri: string; title: string}[] | null } {
...
function renderSources(messageContent: HTMLElement, sources: {uri: string; title: string}[]): void {
...
async function renderMessage(sender: 'user' | 'model', parts: Part[], historyIndex: number): Promise<HTMLElement> {
  ...
  for (const part of parts) {
    if ('text' in part && part.text) {
      ...
      textElement.innerHTML = await marked.parse(cleanText) as string;
      if (sender === 'model') {
...
function createMessageActionToolbar(sender: 'user' | 'model', text: string, historyIndex: number, parts: Part[]): HTMLDivElement {
  ...
  if (sender === 'user') {
    ...
    const wrapper = document.querySelector(`.message-wrapper[data-history-index="${historyIndex}"] .message-bubble`) as HTMLElement;
    readButton.onclick = () => {
...
async function handleFormSubmit(e?: Event): Promise<void> {
  ...
  try {
    ...
    const thinkingBudgetMap: Record<string, number> = { '0': 0, '2': 250, '5': 1500, '10': 5000, '20': 10000, '50': 20000, '100': 24576 };
    ...
    while (keepGenerating) {
        ...
        for await (const chunk of responseStream) {
            ...
            if (chunk && chunk.text) {
              ...
              modelBubble.innerHTML = await marked.parse(fullResponseText + ' ▌') as string;
              responseContainer.scrollTop = responseContainer.scrollHeight;
            }
    ...
    if (isConversationModeActive) {
      ...
      if (lastMessageBubble?.textContent) {
        detectLanguageAndSpeak(lastMessageBubble.textContent, lastMessageBubble as HTMLElement);
      }
    }
  }
}
...
function handleStartEdit(historyIndex: number, parts: Part[]): void {
    const textPart = parts.find(p => 'text' in p);
    if (!textPart || !('text' in textPart)) return; // Can only edit messages with text

    const messageWrapper = responseContainer.querySelector(`.message-wrapper[data-history-index="${historyIndex}"]`) as HTMLDivElement;
...
async function handleRegenerate(): Promise<void> {
  ...
  input.value = (lastUserPrompt.find(p => 'text' in p) as { text: string })?.text || '';
...
async function generateImage(prompt: string): Promise<void> {
  ...
  const userParts: Part[] = [{ text: `/imagine ${prompt}` }];
  const currentHistoryLength = allChats[activeChatId!]?.length || 0;
...
async function handleVoiceCommand(text: string): Promise<void> {
  if (!text || !activeChatId) return;
  toggleWelcomeScreen(false);
  const userParts: Part[] = [{ text: `/voice ${text}` }];
  allChats[activeChatId].push({ role: 'user', parts: userParts });
  saveStateToLocalStorage();
...
function handleVoiceToTextCommand(): void {
  resetFormInput();
  audioFileInput.click();
}

/** Handles audio file selection for transcription */
async function handleAudioFileSelect(event: Event): Promise<void> {
  ...
  reader.onloadend = async () => {
    try {
      ...
      const audioPart: Part = { inlineData: { mimeType: file.type, data: base64String } };
      const promptPart: Part = { text: "Transcribe the following audio file. Provide only the transcribed text." };
      
      const userParts: Part[] = [audioPart];
      const partsForModel: Part[] = [audioPart, promptPart];
      ...
      const modelParts: Part[] = [{ text: transcription }];
...
async function generateAndSaveTitle(chatId: string, history: ChatMessage[]): Promise<void> {
  try {
    const userPrompt = (history[0].parts.find(p => 'text' in p) as { text: string })?.text || 'Media prompt';
    const modelResponsePart = history[1].parts.find(p => 'text' in p);
    const modelResponse = (modelResponsePart as { text: string })?.text || 'Generated a response.';
...
function evaluateExpression(expression: string): string | number {
  try {
...
function showToolUseIndicator(text: string, parentElement: HTMLElement): HTMLElement {
...
function setFormState(disabled: boolean): void {
...
function toggleWelcomeScreen(show: boolean): void {
...
promptStartersContainer.addEventListener('click', (e: MouseEvent) => {
  const target = e.target as HTMLElement;
  const card = target.closest('.prompt-card');
  if (card && (card as HTMLElement).dataset.prompt) {
    input.value = (card as HTMLElement).dataset.prompt!;
    handleFormSubmit();
  }
});
...
responseContainer.addEventListener('click', (e: MouseEvent) => {
  const target = e.target as HTMLElement;
  const editButton = target.closest('.edit-image-button');
  if(editButton) {
    const mediaContainer = editButton.closest('.media-container');
    const image = mediaContainer?.querySelector('.generated-image') as HTMLImageElement;
    if (image) {
...
attachFileButton.addEventListener('click', (e: MouseEvent) => {
    e.stopPropagation();
    attachmentMenu.classList.toggle('hidden');
});
document.addEventListener('click', (e: MouseEvent) => {
    if (!attachmentMenu.classList.contains('hidden') && !attachmentMenu.contains(e.target as Node) && e.target !== attachFileButton) {
...
imageFileInput.addEventListener('change', async (event: Event) => {
    const target = event.target as HTMLInputElement;
...
videoFileInput.addEventListener('change', async (event: Event) => {
    const target = event.target as HTMLInputElement;
    const file = target.files?.[0];
    if (file) {
        try {
...
async function detectLanguageAndSpeak(text: string, bubbleElement: HTMLElement): Promise<void> {
...
function speak(text: string, bubbleElement: HTMLElement, lang: string = 'en-US'): void {
  if (synth.speaking) {
    synth.cancel();
  ...
  utterance.onerror = (event: SpeechSynthesisErrorEvent) => console.error('SpeechSynthesisUtterance.onerror', event);
...
function resetCreativeStudio(): void {
...
function updateStudioGenerateButtonState(): void {
    studioGenerateButton.disabled = !(studioImageFile && studioPromptInput.value.trim());
}

async function handleStudioImageFile(file: File | null | undefined): Promise<void> {
...
studioImageInput.addEventListener('change', (e: Event) => handleStudioImageFile((e.target as HTMLInputElement).files?.[0]));

['dragenter', 'dragover', 'dragleave', 'drop'].forEach(eventName => {
  studioImageDropZone.addEventListener(eventName, (e: DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
  }, false);
});
...
studioImageDropZone.addEventListener('drop', (e: DragEvent) => {
    const dt = e.dataTransfer;
    const file = dt?.files[0];
    if (file) handleStudioImageFile(file);
});

studioRemoveImageButton.addEventListener('click', (e: MouseEvent) => {
    e.stopPropagation();
    studioImageFile = null;
    studioImageInput.value = '';
    studioImagePreview.src = '';
...
function updateEditorGenerateButtonState(): void {
    editorGenerateButton.disabled = !(editorSourceImage && editorPrompt.value.trim());
}

function handleImageEdit(imageElement: HTMLImageElement): void {
    resetImageEditor();
...
editorGenerateButton.addEventListener('click', async () => {
    ...
    try {
        const imagePart: Part = { inlineData: editorSourceImage };
        const textPart: Part = { text: editorPrompt.value.trim() };
...
editorAddToChatButton.addEventListener('click', async () => {
    ...
    const modelParts: Part[] = [{ inlineData: editorGeneratedImage }];
